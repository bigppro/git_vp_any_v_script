...
echo "?Que version de Odoo se instalara (ej. 12.0)?"
read OE_VERSION 
  # OE_VERSION="14.0"
...
# Install PDF (wkhtmltopdf)
#--- sudo wget https://github.com/wkhtmltopdf/wkhtmltopdf/releases/download/0.12.5/wkhtmltox_0.12.5-1.bionic_amd64.deb
#--- echo y | sudo apt-get install ./wkhtmltox_0.12.5-1.bionic_amd64.deb
...
# Install ODOO
#--------------------------------------------------
echo -e "\n==== Installing ODOO V"$OE_VERSION "Server ===="
...
if [ $IS_ENTERPRISE = "True" ]; then
    sudo su root -c "printf 'addons_path=${OE_HOME}/enterprise/addons,${OE_HOME_EXT}/odoo/addons\n' >> /etc/${OE_CONFIG}.conf"
else
    sudo su root -c "printf 'addons_path=${OE_HOME_EXT}/addons,${OE_HOME_EXT}/odoo/addons,${OE_HOME}/custom-addons\n' >> /etc/${OE_CONFIG}.conf"
fi
...
if [ $INSTALL_NGINX = "True" ] && [ $ENABLE_SSL = "True" ] && [ $SSL_ADMIN_EMAIL != "odoo@example.com" ]  && [ $WEBSITE_NAME != "_" ];then
  sudo add-apt-repository ppa:certbot/certbot -y && sudo apt-get update -y
  sudo apt-get install python3-certbot-nginx -y
  sudo certbot --nginx -d $WEBSITE_NAME --noninteractive --agree-tos --email $SSL_ADMIN_EMAIL --redirect
  sudo service nginx reload
...
echo "Configuring odoo as a service (Sudo service odoo start/restart/stop/status)"
...
User=odoo
Group=odoo
ExecStart=$OE_HOME_EXT/odoo-bin --c /etc/${OE_CONFIG}.conf --logfile /var/log/$OE_USER/$OE_CONFIG.log
KillMode=mixed
...
echo -e "\n Security odoo.service\n"

#Step 3 : Security Init File

sudo mv ~/$OE_CONFIG.service /lib/systemd/system/$OE_CONFIG.service

sudo chmod 755 /lib/systemd/system/$OE_CONFIG.service

sudo chown root:odoo /lib/systemd/system/$OE_CONFIG.service
...
sudo tail -f /var/log/$OE_USER/$OE_CONFIG.log
